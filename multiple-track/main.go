...
func handleConnections(w http.ResponseWriter, r *http.Request) {
	...
	peerConnection.OnTrack(func(track *webrtc.TrackRemote, receiver *webrtc.RTPReceiver) {
		...
		go func() {
			playFromDisk("output.ogg", peerConnection, 0)
			time.Sleep(3 * time.Second)
			playFromDisk("output.ogg", peerConnection, 1)
		}()
		for {
...
func playFromDisk(audioFileName string, pc *webrtc.PeerConnection, seq int) {
	...
	if haveAudioFile {
		id := fmt.Sprintf("audio%d", seq)
		audioTrack, audioTrackErr := webrtc.NewTrackLocalStaticRTP(webrtc.RTPCodecCapability{MimeType: webrtc.MimeTypeOpus}, id, id)
